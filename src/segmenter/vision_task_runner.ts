...
import * as rect_pb from './rect_pb.js';
const NormalizedRect = (rect_pb as any).default.NormalizedRect;
import {TaskRunner} from './task_runner.js';
import {WasmFileset} from './wasm_fileset.js';
import {MPImage} from './image.js';
import {ImageProcessingOptions} from './image_processing_options.js';
import {MPImageShaderContext} from './image_shader_context.js';
import {MPMask} from './mask.js';
import {getImageSourceSize, GraphRunner, ImageSource, WasmMediaPipeConstructor} from './graph_runner.js';
import {SupportImage, WasmImage} from './graph_runner_image_lib.js';
import {supportsOffscreenCanvas} from './platform_utils.js';
import {SupportModelResourcesGraphService} from './register_model_resources_graph_service.js';

import {VisionTaskOptions} from './vision_task_options.js';
...
export abstract class VisionTaskRunner extends TaskRunner {
  ...
  private convertToNormalizedRect(
      ...
      imageProcessingOptions?: ImageProcessingOptions): any {
    ...
    if (data instanceof Uint8Array) {
      if (data.length === pixels * 3) {
        ...
        container = new ImageData(
            new Uint8ClampedArray(data.buffer, data.byteOffset, data.length) as any,
            width, height);
      } else {
